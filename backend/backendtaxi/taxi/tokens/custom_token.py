...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def get_token(cls, user):
        ...
        if user.is_driver:
            token['driver_profile_id'] = user.driver_profile.id if user.driver_profile else None
        if user.is_rider:
            token['rider_profile_id'] = user.rider_profile.id if user.rider_profile else None
    ...
    def validate(self, attrs):
        ...
        if self.user.is_driver and self.user.driver_profile:
            driver_object = {
